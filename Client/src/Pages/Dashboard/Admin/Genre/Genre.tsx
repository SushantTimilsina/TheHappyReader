...
import { deleteGenre, getGenreData } from "store/genre/genre-actions";
...
const Genre = () => {
  ...
  const { error, loading, genre, prev, next } = useSelector(
    (state: any) => state.genre
  );

  // fetching genre data
  useEffect(() => {
    dispatch(getGenreData(page, 5));
  }, [dispatch, page]);

  // canceling delete genre
    ...
    <>
      ...
      <DashboardLayout>
        ...
        <div className="flex flex-col md:flex-row justify-between my-8 md:mx-12 lg:mx-24 lg:max-w-[1100px]">
          <h1 className="text-xl mb-4 font-bold ">Genre List</h1>
          <Button className="max-w-fit rounded-md">
            <Link to="/admin-dashboard/genre/add">Add</Link>
          </Button>
        </div>
        {/* header and button ends */}

        {/* genre listing starts */}
        <Card className="mx-4 p-0 md:mx-12 lg:mx-24 lg:max-w-[1100px] rounded-md my-8 pb-14 relative">
          ...
          {genre?.length === 0 ? (
            ...
            <>
              {genre?.map((data: GenreInterface, index: number) => {
                const { name, _id } = data;
                return (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <div className="flex gap-4 text-xl">
                      <Link
                        ...
                        to={`/admin-dashboard/genre/edit?genreId=${_id}`}
                      >
                        <AiFillEdit />
                      </Link>
                      <span
